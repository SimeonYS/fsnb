...
class FfsnbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fsnb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fsnb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fsnb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fsnb (date, title, link, content)"
